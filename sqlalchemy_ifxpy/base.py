# sqlalchemy_ifxpy/base.py
# Copyright (C) 2005-2013 the SQLAlchemy authors and contributors <see AUTHORS file>
# Copyright (C) 2018 Tim Powell
# coding: gbk
#
...
.. note::
    ...
    This Informix dialect functions on current SQLAlchemy versions
    but has been tested for select statements only
...
class InfoDDLCompiler(compiler.DDLCompiler):
    ...
    ### Informix wants the constraint name at the end, hence this is c&p from sql/compiler.py
...
class InformixDialect(default.DefaultDialect):
    ...
    _isolation_levels = set(['DIRTY READ', 'REPEATABLE READ', 'COMMITTED READ',
            'CURSOR STABILITY'])

    def set_isolation_level(self, connection, level):
        if self.has_transactions:
            if level is None or len(level.strip()) < 1:
                # Default for non ANSI databases with logging
                level = 'COMMITTED READ'
            level.upper().replace("-", " ")
            ...
            cu = connection.cursor()
            cu.execute("SET ISOLATION %s" % level)
            cu.execute("COMMIT")
            cu.close()
